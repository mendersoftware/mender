...
import (
	...
	"github.com/mendersoftware/mender/common/conf"
...
func getDefaultDeviceType(ctx *cli.Context) (devType string) {
	devType, err := installer.GetDeviceType(path.
...
func doSetup(ctx *cli.Context, config *combinedConfig,
...
func (opts *setupOptionsType) saveConfigOptions(config *combinedConfig) error {
	if opts.demo {
	...
	config.Servers = []authconf.MenderServer{
		{
	...
	if err := saveConfigFile(config, opts.configPath); err != nil {
		return err
	}
...
func saveConfigFile(config *combinedConfig, filename string) error {
