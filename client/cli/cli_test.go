...
func writeConfig(t *testing.T, path string, config *combinedConfig) {
...
func TestMainBootstrap(t *testing.T) {
	...
	writeConfig(t, cpath, &combinedConfig{
		AuthConfig: &authconf.AuthConfig{
			Servers: []authconf.MenderServer{{ServerURL: ts.URL}},
			Config: commonconf.Config{
				DBus: commonconf.DBusConfig{
					Enabled: true,
				},
			},
		},
...
`)
	...
	keyold, err := ds.ReadAll(commonconf.DefaultKeyFile)
	assert.NoError(t, err)
	...
	keynew, err := ds.ReadAll(commonconf.DefaultKeyFile)
	assert.NoError(t, err)
...
func TestPrintArtifactName(t *testing.T) {
	...
	deviceManager := installer.NewDeviceManager(nil, config, dbstore)
	...
	dbstore.WriteAll(dbkeys.ArtifactNameKey, []byte("db-name"))
	...
	dbstore.Remove(dbkeys.ArtifactNameKey)
...
func TestPrintProvides(t *testing.T) {
	...
	// artifact name
	dbstore.WriteAll(dbkeys.ArtifactNameKey, []byte("name"))
	...
	dbstore.WriteAll(dbkeys.ArtifactNameKey, []byte("name"))
	dbstore.WriteAll(dbkeys.ArtifactGroupKey, []byte("group"))
	...
	assert.NoError(t, err)
	dbstore.WriteAll(dbkeys.ArtifactTypeInfoProvidesKey, typeInfoProvidesBuf)
...
func TestInvalidServerCertificateBoot(t *testing.T) {
	...
	_, _, err = initDaemon(&mconf, &runOptionsType{dataStore: tdir, bootstrapForce: false})
...
func TestIgnoreServerConfigVerification(t *testing.T) {
	...
	commonconf.DefaultConfFile = cpath
	...
	commonconf.DefaultDataStore = tdir
