...
func initCLITest(t *testing.T, flagSet *flag.FlagSet) (*cli.Context,
	*combinedConfig, *runOptionsType) {
	...
	assert.NoError(t, err)
	err = commonconf.LoadConfig(confPath, "", config.AuthConfig)
	assert.NoError(t, err)
	sysConfig := &config.MenderConfig.MenderConfigFromFile
