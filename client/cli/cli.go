...
// This struct exists specifically to handle some of the mender client commands
// where the responsibilities of the updater and the auth manager overlap. It
// should not be used throughout the rest of the client, and is therefore not
// added to the "conf" package.
...
func SetupCLI(args []string) error {
		...
		},
		{
			...
			Flags: []cli.Flag{
				&cli.StringFlag{
					...
					Value:       commonconf.DefaultConfFile,
					Usage:       "`PATH` to configuration file."},
				&cli.StringFlag{
					Name:    "data",
					Aliases: []string{"d"},
					Usage:   "Mender state data `DIR`ECTORY path.",
					Value:   commonconf.DefaultDataStore},
				&cli.StringFlag{
					Name:        "device-type",
	...
	app.Flags = []cli.Flag{
		...
		&cli.StringFlag{
			...
			Value:       commonconf.DefaultFallbackConfFile,
...
func (runOptions *runOptionsType) commonCLIHandler(
	ctx *cli.Context) (*combinedConfig, error) {
	...
	err := commonconf.LoadConfig(runOptions.config,
		runOptions.fallbackConfig, config.MenderConfig)
	if err != nil {
		return nil, err
	}
	err = commonconf.LoadConfig(runOptions.config,
		runOptions.fallbackConfig, config.AuthConfig)
	if err != nil {
		return nil, err
	}

	// Make sure that paths that are not configurable via the config file is consistent with --data flag
...
func (runOptions *runOptionsType) handleCLIOptions(ctx *cli.Context) error {
	...
	case "show-artifact",
		...
		"commit",
		"rollback":
		return handleArtifactOperations(ctx, *runOptions, config.MenderConfig)

	case "bootstrap":
		return doBootstrapAuthorize(config, runOptions)

	case "daemon":
		d, authmgr, err := initDaemon(config, runOptions)
		if err != nil {
			return err
		}
		defer d.Cleanup()
		return runDaemon(d, authmgr)
	case "setup":
...
func (c *combinedConfig) Validate() error {
	// No validator for MenderConfigFromFile, currently.

	return c.AuthConfig.Validate()
}
