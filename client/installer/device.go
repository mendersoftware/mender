...
type DeviceManager struct {
	...
	Installers          []PayloadUpdatePerformer
	InstallerFactories  AllModules
	StateScriptExecutor statescript.Executor
	StateScriptPath     string
	Store               store.Store
}

func NewDeviceManager(dualRootfsDevice DualRootfsDevice, config *conf.MenderConfig, store store.Store) *DeviceManager {
	d := &DeviceManager{
	...
	d.InstallerFactories = AllModules{
		DualRootfs: dualRootfsDevice,
		Modules: NewModuleInstallerFactory(config.ModulesPath,
...
func (d *DeviceManager) GetCurrentArtifactName() (string, error) {
	if d.Store != nil {
		dbname, err := d.Store.ReadAll(dbkeys.ArtifactNameKey)
		if err == nil {
			name := string(dbname)
...
func (d *DeviceManager) ReadArtifactHeaders(from io.ReadCloser) (*Installer, error) {
	...
	var i *Installer
	i, d.Installers, err = ReadHeaders(from,
		deviceType,
...
func (d *DeviceManager) GetInstallers() []PayloadUpdatePerformer {
	return d.Installers
}

func (d *DeviceManager) RestoreInstallersFromTypeList(payloadTypes []string) error {
	var err error
	d.Installers, err = CreateInstallersFromList(&d.InstallerFactories, payloadTypes)
	return err
}
