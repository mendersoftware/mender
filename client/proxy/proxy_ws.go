...
var (
	// From https://github.com/mendersoftware/deviceconnect/blob/master/api/http/device.go
...
func (pc *ProxyController) DoWsUpgrade(w http.ResponseWriter, r *http.Request) {
	...
	// Only pass these headers to the upgrader.
	...
	go pc.forwardWsConnection(newConn.connClient, newConn.connBackend, errClient, &newConn.connClientWriteMutex)
	go pc.forwardWsConnection(newConn.connBackend, newConn.connClient, errBackend, &newConn.connBackendWriteMutex)
...
func (pc *ProxyController) forwardWsConnection(dst, src *websocket.Conn, errChann chan error, mutex *sync.Mutex) {
	for {
...
func (pc *ProxyController) wsAvailable() bool {
	pc.wsConnectionsMutex.Lock()
	defer pc.wsConnectionsMutex.Unlock()
...
func (pc *ProxyController) CloseWsConnections() {
	...
	pc.wsConnectionsMutex.Lock()
	defer pc.wsConnectionsMutex.Unlock()
	for c, _ := range pc.wsConnections {
