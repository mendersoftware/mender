...
func prepareProxyWsTest(t *testing.T, srv *cltest.ClientTestWsServer) (*ProxyController, *websocket.Conn) {
...
func TestProxyWsConnect(t *testing.T) {
	...
	srv.Connect.SendMessages = append(srv.Connect.SendMessages, cltest.WsMessage{MsgType: websocket.TextMessage, Msg: []byte("one")})
	srv.Connect.SendMessages = append(srv.Connect.SendMessages, cltest.WsMessage{MsgType: websocket.TextMessage, Msg: []byte("two")})
	srv.Connect.SendMessages = append(srv.Connect.SendMessages, cltest.WsMessage{MsgType: websocket.TextMessage, Msg: []byte("three")})
	srv.Connect.SendMessages = append(srv.Connect.SendMessages, cltest.WsMessage{MsgType: websocket.TextMessage, Msg: []byte("five")})
...
func TestProxyWsTooMany(t *testing.T) {
	...
	testUrl := strings.Replace(proxyController.GetServerUrl(), "http:", "ws:", 1) + ApiUrlDevicesConnect
	headers := http.Header{}
	headers.Add("Authorization", "Bearer SecretJwtToken")
...
func TestProxyWsGoroutines(t *testing.T) {
	...
	log.Infof("goRoutines start %d proxy %d open %d close %d stop %d", goRoutinesStart, goRoutinesProxyStart, goRoutinesConnOpen, goRoutinesConnClose, goRoutinesStop)
}
