...
type InventorySubmitter interface {
	Submit(api ApiRequester, data interface{}) error
}
...
func (i *InventoryClient) Submit(api ApiRequester, data interface{}) error {
	// PATCH used to be the only method available in Mender Product 2.5, so
	// fall back to that if PUT fails.
	r, err := doSubmitInventory(api, http.MethodPut, data)
	if r != nil && r.StatusCode == http.StatusMethodNotAllowed {
		r, err = doSubmitInventory(api, http.MethodPatch, data)
	}
...
func doSubmitInventory(api ApiRequester, method string, data interface{}) (*http.Response, error) {
	req, err := makeInventorySubmitRequest(method, data)
	if err != nil {
	...
	if r.StatusCode != http.StatusOK {
		return r, common.NewAPIError(errors.Errorf("Got unexpected HTTP status when submitting to inventory %d", r.StatusCode), r)
	}
	return r, nil
}

func makeInventorySubmitRequest(method string, data interface{}) (*http.Request, error) {
	url := BuildApiURL("/v1/inventory/device/attributes")
