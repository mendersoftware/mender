...
type Updater interface {
	GetScheduledUpdate(api ApiRequester, current *CurrentUpdate) (interface{}, error)
...
func (u *UpdateClient) GetScheduledUpdate(api ApiRequester,
	current *CurrentUpdate) (interface{}, error) {

	return u.getUpdateInfo(api, processUpdateResponse, current)
}
...
func (u *UpdateClient) getUpdateInfo(api ApiRequester, process RequestProcessingFunc,
	current *CurrentUpdate) (interface{}, error) {
	reqs, err := makeUpdateCheckRequest(current)
	if err != nil {
	...
	if err != nil {
		r.Body = ioutil.NopCloser(bytes.NewReader(respdata))
		return data, common.NewAPIError(err, r)
	}
	return data, err
}
...
func (u *UpdateClient) FetchUpdate(api ApiRequester, url string, maxWait time.Duration) (io.ReadCloser, int64, error) {
	...
	if r.StatusCode != http.StatusOK {
		err = common.NewAPIError(errors.New("error receiving scheduled update information"), r)
		r.Body.Close()
...
func makeUpdateCheckRequest(current *CurrentUpdate) ([]*http.Request, error) {
