...
type LogUploader interface {
	Upload(api ApiRequester, logs LogData) error
}
...
func (u *LogUploadClient) Upload(api ApiRequester, logs LogData) error {
	req, err := makeLogUploadRequest(&logs)
	if err != nil {
	...
	if r.StatusCode != http.StatusNoContent {
		...
		return common.NewAPIError(errors.Errorf("uploading logs failed, bad status %v", r.StatusCode), r)
	}
...
func makeLogUploadRequest(logs *LogData) (*http.Request, error) {
