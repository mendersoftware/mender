...
func LoadProvides(dbStore store.Store) (map[string]string, error) {
	...
	err = dbStore.ReadTransaction(func(txn store.Transaction) error {
		...
		providesBuf, err = txn.ReadAll(dbkeys.ArtifactNameKey)
		...
		providesBuf, err = txn.ReadAll(dbkeys.ArtifactGroupKey)
		if err != nil && !os.IsNotExist(err) {
			return errors.Wrapf(err, errMsgReadingFromStoreF,
				"ArtifactGroup")
		} else if err == nil {
		...
		providesBuf, err = txn.ReadAll(
			dbkeys.ArtifactTypeInfoProvidesKey)
		if err != nil && !os.IsNotExist(err) {
			return errors.Wrapf(err, errMsgReadingFromStoreF,
...
func StoreStateDataAndTransaction(dbStore store.Store, sd StateData,
	...
	err := dbStore.WriteTransaction(func(txn store.Transaction) error {
		...
		if sd.UpdateInfo.HasDBSchemaUpdate {
			key = dbkeys.StateDataKeyUncommitted
		} else {
			key = dbkeys.StateDataKey
		}
		...
		if !sd.UpdateInfo.HasDBSchemaUpdate {
			err = txn.Remove(dbkeys.StateDataKeyUncommitted)
			if err != nil {
				return err
			}
		}
...
func LoadStateData(dbStore store.Store) (StateData, error) {
	...
	err := dbStore.WriteTransaction(func(txn store.Transaction) error {
		var err error
		sd, err = loadStateData(txn, dbkeys.StateDataKey)
		if err != nil {
			return err
		}
		...
		case 0, 1:
			...
			uncommSd, err := loadStateData(txn, dbkeys.StateDataKeyUncommitted)
			if err == nil {
		...
		if sd.UpdateInfo.HasDBSchemaUpdate {
			return txn.WriteAll(dbkeys.StateDataKeyUncommitted, data)
		}
		return txn.WriteAll(dbkeys.StateDataKey, data)
	})
...
func CommitArtifactData(txn store.Transaction, artifactName, artifactGroup string,
	...
	if err = txn.WriteAll(dbkeys.ArtifactNameKey, []byte(artifactName)); err != nil {
		return err
	}
	...
	if err = txn.WriteAll(dbkeys.ArtifactTypeInfoProvidesKey, providesBuf); err != nil {
		return err
	}
	...
	if artifactGroup == "" {
		...
		if removeGroup {
			err = txn.Remove(dbkeys.ArtifactGroupKey)
			if err != nil {
				return err
			}
		}
		...
		err = txn.WriteAll(dbkeys.ArtifactGroupKey, []byte(artifactGroup))
		if err != nil {
			return err
		}
	}
...
func getProvidesToPreserve(txn store.Transaction, clearsProvides []string) (map[string]string, error) {
	...
	providesBuf, err := txn.ReadAll(dbkeys.ArtifactTypeInfoProvidesKey)
