...
func loadServerTrust(ctx *openssl.Ctx, conf *Config) (*openssl.Ctx, error) {
	...
	if err != nil {
		return ctx, errors.Wrap(
			err,
			"Failed to get the default OpenSSL certificate directory. Please verify the OpenSSL setup",
		)
	}
...
func (h *HttpsClient) Validate() {
	...
	if h.Certificate != "" || h.Key != "" {
		if h.Certificate == "" {
			log.Error(
				"The 'Key' field is set in the mTLS configuration, but no 'Certificate' is given. Both need to be present in order for mTLS to function",
			)
		}
		if h.Key == "" {
			log.Error(
				"The 'Certificate' field is set in the mTLS configuration, but no 'Key' is given. Both need to be present in order for mTLS to function",
			)
