...
func getTestDeviceManager(dualRootfsDevice installer.DualRootfsDevice,
	config *conf.MenderConfig, deviceTypeFile string, dbdir string) *installer.DeviceManager {

	dbstore := store.NewDBStore(dbdir)

	dm := installer.NewDeviceManager(dualRootfsDevice, config, dbstore)
...
func standaloneInstallSetup(t *testing.T, tmpdir string,
	tma *tests.TestModuleAttr, aao tests.ArtifactAttributeOverrides) (*installer.DeviceManager,
	...
	device := installer.NewDeviceManager(nil, &config, dbstore)
...
func Test_doManualUpdate_noParams_fail(t *testing.T) {
	...
	if err := DoStandaloneInstall(getTestDeviceManager(dualRootfsDevice, &config, deviceType, dbdir),
		"", tls.Config{}, nil, installer.NewStateScriptExecutor(&config), false); err == nil {
...
func Test_doManualUpdate_invalidHttpsClientConfig_updateFails(t *testing.T) {
	...
	if err := DoStandaloneInstall(getTestDeviceManager(
		...
		installer.NewStateScriptExecutor(&config), false); err == nil {
...
func Test_doManualUpdate_networkUpdateNoClient_fail(t *testing.T) {
	...
	if err := DoStandaloneInstall(getTestDeviceManager(fakeDevice, &config, deviceType, dbdir),
		imageFile, tls.Config{}, nil, installer.NewStateScriptExecutor(&config), false); err == nil {
...
func Test_doManualUpdate_existingFile_updateSuccess(t *testing.T) {
	...
	err = DoStandaloneInstall(getTestDeviceManager(
		...
		imageFileName, tls.Config{},
		nil, installer.NewStateScriptExecutor(&config), false)
	assert.NoError(t, err)
}
...
func Test_doManualUpdate_existingFile_updateSuccess_rebootExitCode(t *testing.T) {
	...
	if err = DoStandaloneInstall(getTestDeviceManager(
		fakeDev, &config, deviceType, dbdir),
		imageFileName, tls.Config{},
		nil, installer.NewStateScriptExecutor(&config), true); err != ErrorManualRebootRequired {
		t.FailNow()
	}
}
...
func TestDoManualUpdateArtifactV3Dependencies(t *testing.T) {
	...
	testDevMgr.Store.WriteAll(
		dbkeys.ArtifactTypeInfoProvidesKey, []byte("null"))
	err = DoStandaloneInstall(testDevMgr,
		imageFileName, tls.Config{},
		nil, installer.NewStateScriptExecutor(&config), false)
	assert.Error(t, err)
	...
	testDevMgr.Store.WriteAll(dbkeys.ArtifactNameKey,
		[]byte("OldArtifact"))
	err = DoStandaloneInstall(testDevMgr,
		imageFileName, tls.Config{},
		nil, installer.NewStateScriptExecutor(&config), false)
	...
	testDevMgr.Store.WriteAll(dbkeys.ArtifactGroupKey,
		[]byte("testGroup"))
	err = DoStandaloneInstall(testDevMgr,
		imageFileName, tls.Config{},
		nil, installer.NewStateScriptExecutor(&config), false)
	...
	assert.NoError(t, err)
	testDevMgr.Store.WriteAll(
		dbkeys.ArtifactTypeInfoProvidesKey, typeProvidesBuf)
	err = DoStandaloneInstall(testDevMgr,
		imageFileName, tls.Config{},
		nil, installer.NewStateScriptExecutor(&config), false)
	assert.NoError(t, err)
	...
	for _, c := range standaloneModuleInstallCases {
		t.Run(c.caseName, func(t *testing.T) {
			...
			err = DoStandaloneInstall(device, artPath,
				tls.Config{}, nil, stateExec, false)
			if c.errInstall != "" {
				require.Error(t, err)
	...
	for _, c := range testCases {
		t.Run(c.caseName, func(t *testing.T) {
			...
			if hasArtifactGroup {
				...
				err = device.Store.WriteAll(dbkeys.ArtifactGroupKey, []byte(artifactGroup))
				require.NoError(t, err)
			...
			if c.preexistingProvides != nil {
				...
				require.NoError(t, err)
				err = device.Store.WriteAll(dbkeys.ArtifactTypeInfoProvidesKey, jsonBytes)
				require.NoError(t, err)
			}
