...
func TestStateUpdateReportStatus(t *testing.T) {
	...
	usr := NewUpdateStatusReportState(update, api.StatusFailure)
	usr.Handle(&ctx, sc)
	assert.Equal(t, api.StatusFailure, sc.reportStatus)
	...
	sc = &stateTestController{}
	usr = NewUpdateStatusReportState(update, api.StatusSuccess)
	usr.Handle(&ctx, sc)
	assert.Equal(t, api.StatusSuccess, sc.reportStatus)
	assert.Equal(t, *update, sc.reportUpdate)
	...
	s = NewUpdateStatusReportState(update, api.StatusSuccess)
	...
	s = NewUpdateStatusReportState(update, api.StatusFailure)
	...
	usr = NewUpdateStatusReportState(update, api.StatusSuccess)
	sc = &stateTestController{
		reportError: NewFatalError(api.ErrDeploymentAborted),
	}
...
func TestStateUpdateCommit(t *testing.T) {
	...
	storeBuf, err = ms.ReadAll(dbkeys.ArtifactGroupKey)
	assert.NoError(t, err)
	artifactGroup := string(storeBuf)
	assert.Equal(t, artifactGroup, "TestGroup")
	storeBuf, err = ms.ReadAll(dbkeys.ArtifactTypeInfoProvidesKey)
	assert.NoError(t, err)
...
func TestStateUpdateCheck(t *testing.T) {
	...
	// pretend we have an update
	s, c = cs.Handle(ctx, &stateTestController{
		updateRespErr: NewTransientError(api.ErrNoDeploymentAvailable),
	})
	assert.IsType(t, &checkWaitState{}, s)
	assert.False(t, c)
}
...
func TestUpdateCheckSameImage(t *testing.T) {
	...
	assert.Equal(t, api.StatusAlreadyInstalled, urs.status)
}
...
func TestStateUpdateFetch(t *testing.T) {
	...
	assert.IsType(t, &updateStoreState{}, s)
	assert.False(t, c)
	assert.Equal(t, api.StatusDownloading, sc.reportStatus)
	assert.Equal(t, *update, sc.reportUpdate)
...
func TestStateUpdateStore(t *testing.T) {
	...
	ms.WriteAll(dbkeys.ArtifactNameKey, []byte("OtherArtifact"))
	ms.WriteAll(dbkeys.ArtifactGroupKey, []byte("TestGroup"))
...
func TestUpdateStoreDependencies(t *testing.T) {
	...
	assert.IsType(t, &updateStatusReportState{}, s)
	assert.False(t, c)
	ms.WriteAll(dbkeys.ArtifactNameKey, []byte("NonExistingArtie"))
	...
	assert.IsType(t, &updateStatusReportState{}, s)
	assert.False(t, c)
	ms.WriteAll(dbkeys.ArtifactGroupKey, []byte("WrongGroup"))
	stream.Seek(0, io.SeekStart)
	s, c = uis.Handle(&ctx, sc)
	assert.IsType(t, &updateStatusReportState{}, s)
	assert.False(t, c)
...
func TestStateUpdateInstallFailed(t *testing.T) {
	...
	uis := NewUpdateStoreState(stream, update)
	ms := store.NewMemStore()
	ms.WriteAll(dbkeys.ArtifactNameKey, []byte("preexisting-name"))
	ctx := StateContext{
		Store: ms,
	}
	stc := stateTestController{
...
func TestStateData(t *testing.T) {
	...
	data, err := ms.ReadAll(dbkeys.StateDataKey)
	assert.NoError(t, err)
...
var stateTransitionsWithUpdateModulesTestCases []stateTransitionsWithUpdateModulesTestCase = []stateTransitionsWithUpdateModulesTestCase{
	...
	stateTransitionsWithUpdateModulesTestCase{
		...
		failStatusReportStatus: api.StatusInstalling,
	},
...
func subProcessSetup(t *testing.T,
	...
	api.ExponentialBackoffSmallestUnit = time.Millisecond
...
func TestDBSchemaUpdate(t *testing.T) {
	...
	_, err = db.ReadAll(dbkeys.StateDataKeyUncommitted)
	assert.Error(t, err)
	_, err = db.ReadAll(dbkeys.StateDataKey)
	assert.NoError(t, err)
	...
	data, err := db.ReadAll(dbkeys.StateDataKeyUncommitted)
	require.NoError(t, err)
	...
	data, err = db.ReadAll(dbkeys.StateDataKey)
	require.NoError(t, err)
	err = json.Unmarshal(data, &sd)
	require.NoError(t, err)
...
func TestControlMapFetch(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Err: deployment aborted": {
			updateCheckError:  NewTransientError(api.ErrNoDeploymentAvailable),
