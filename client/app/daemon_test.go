...
func (f fakeUpdater) GetScheduledUpdate(api api.ApiRequester, url string) (interface{}, error) {
	return f.GetScheduledUpdateReturnIface, f.GetScheduledUpdateReturnError
}
func (f fakeUpdater) FetchUpdate(api api.ApiRequester, url string) (io.ReadCloser, int64, error) {
...
func TestDaemon(t *testing.T) {
	...
	d, err := NewDaemon(&conf.MenderConfig{}, mender, store)
...
func TestDaemonCleanup(t *testing.T) {
	...
	require.NoError(t, err)
	d, err := NewDaemon(&conf.MenderConfig{}, mender, mstore)
	require.NoError(t, err)
	d.Cleanup()
...
func TestDaemonRun(t *testing.T) {
	t.Run("Testrun daemon", func(t *testing.T) {
		...
		daemon, err := NewDaemon(&conf.MenderConfig{}, dtc, store.NewMemStore())
		require.NoError(t, err)
