...
func (id *InventoryDataRunner) Get() (api.InventoryData, error) {
...
type InventoryDataDecoder struct {
	data map[string]api.InventoryAttribute
}

func NewInventoryDataDecoder() *InventoryDataDecoder {
	return &InventoryDataDecoder{
		make(map[string]api.InventoryAttribute),
	}
}

func (id *InventoryDataDecoder) GetInventoryData() api.InventoryData {
	if len(id.data) == 0 {
		return nil
	}
	idata := make(api.InventoryData, 0, len(id.data))
...
func (id *InventoryDataDecoder) AppendFromRaw(raw map[string][]string) {
	for k, v := range raw {
		if data, ok := id.data[k]; ok {
			...
			id.data[k] = api.InventoryAttribute{
				Name:  k,
				Value: newVal,
			}
		} else {
