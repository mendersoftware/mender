...
const (
	...
	promptHostedMender = "\nAre you connecting this device to Hosted Mender? [Y/n] "
	promptCredentials  = "\nEnter your Hosted Mender credentials"
...
func (opts *setupOptionsType) askHostedMender(ctx *cli.Context,
	...
	if opts.hostedMender {
		...
		state = stateHostedMenderCredentials
	} else {
		state = stateDemoMode
	}
	return state, nil
...
func doSetup(ctx *cli.Context, config *menderSysConfig,
	...
	for state != stateDone {
		...
		case stateHostedMenderCredentials:
...
func (opts *setupOptionsType) saveConfigOptions(config *menderSysConfig) error {
	if opts.demo {
	...
	if config.DeviceTypeFile == "" {
		...
		config.DeviceTypeFile = defaultDeviceTypeFile
	}
	...
	if err := storeConfigFile(config, opts.configPath); err != nil {
		return err
	}
