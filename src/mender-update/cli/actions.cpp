...
static error::Error ResultHandler(standalone::ResultAndError result) {
	...
	using r = Result;
	auto contains = [&result](r val) {
		return ResultContains(result.result, val);
	};
	auto none_of = [&result](r val) {
		return ResultNoneOf(result.result, val);
	};
	...
	if (contains(r::Committed) and none_of(r::Installed)) {
		...
		operation_done = "Installed, but not committed.\n"
	...
	string &operation = (contains(r::Failed) and none_of(r::FailedInPostCommit | r::CleanupFailed)) ?
		operation_failure : operation_done;

	string additional;

	if (contains(r::RollbackFailed)) {
		additional = "Rollback failed. "
			"System may be in an inconsistent state.";
	} else if (contains(r::NoRollback)) {
		additional = "Update Module does not support rollback. "
			"System may be in an inconsistent state.";
