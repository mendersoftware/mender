...
TEST(JsonStringTests, LoadFromInvalidString) {
	...
	EXPECT_FALSE(ej);
	EXPECT_EQ(ej.error().code, json::JsonErrorCode::ParseError);
...
TEST(JsonDataTests, GetJsonData) {
	...
	ASSERT_FALSE(echild);
	EXPECT_EQ(echild.error().code, json::JsonErrorCode::KeyError);
	...
	echild = j_arr.Get(5);
	ASSERT_FALSE(echild);
	EXPECT_EQ(echild.error().code, json::JsonErrorCode::IndexError);
...
TEST(JsonDataTests, GetDataValues) {
	...
	ASSERT_FALSE(eint);
	EXPECT_EQ(eint.error().code, json::JsonErrorCode::TypeError);
	EXPECT_EQ(eint.error().message, "Type mismatch when getting int");

	json::ExpectedBool ebool = echild.value().GetBool();
	ASSERT_FALSE(ebool);
	EXPECT_EQ(ebool.error().code, json::JsonErrorCode::TypeError);
	...
	ASSERT_TRUE(echild);
	esize = echild.value().GetArraySize();
	EXPECT_EQ(esize.error().code, json::JsonErrorCode::TypeError);
