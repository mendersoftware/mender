...
TEST(CryptoTest, TestPrivateKeyLoadFromPEMFileNotFound) {
	...
	string private_key_file = "./private-key.non-existent.pem";
	auto expected_private_key = PrivateKey::LoadFromPEM(private_key_file);
	EXPECT_THAT(expected_private_key.error().message, testing::HasSubstr("no such file"));
}

TEST(CryptoTest, TestPrivateKeyLoadFromPEMInvalid) {
	// Load currupted/unsupported private key from PEM
	string private_key_file = "./private-key.bogus.pem";
	auto expected_private_key = PrivateKey::LoadFromPEM(private_key_file);
	EXPECT_THAT(expected_private_key.error().message, testing::HasSubstr("unsupported"));
}
...
TEST(CryptoTest, TestPrivateKeyLoadFromPEMNoPassphrase) {
	...
	auto expected_private_key = PrivateKey::LoadFromPEM(private_key_file);
	EXPECT_THAT(expected_private_key.error().message, testing::HasSubstr("bad decrypt"));
}
