...
#if MENDER_USE_LMDB
	auto err = mender_store_.Open(conf::paths::Join(config_.data_store_dir, "mender-store"));
...
expected::ExpectedString MenderContext::GetDeviceType() {
	string device_type_fpath = conf::paths::Join(config_.data_store_dir, "device_type");
