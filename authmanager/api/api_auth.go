...
type AuthRequester interface {
	Request(client *http.Client, server string, dataSrc AuthDataMessenger) ([]byte, error)
}
...
func (u *AuthClient) Request(client *http.Client, server string, dataSrc AuthDataMessenger) ([]byte, error) {
	...
	case http.StatusUnauthorized:
		return nil, api.NewAPIError(AuthErrorUnauthorized, rsp)
	case http.StatusOK:
		log.Debugf("Receive response data")
		data, err := ioutil.ReadAll(rsp.Body)
		if err != nil {
			return nil, api.NewAPIError(errors.Wrapf(err, "failed to receive authorization response data"), rsp)
		}

		log.Debugf("Received response data:  %v", data)
		return data, nil
	default:
		return nil, api.NewAPIError(errors.Errorf("unexpected authorization status %v", rsp.StatusCode), rsp)
	}
}
