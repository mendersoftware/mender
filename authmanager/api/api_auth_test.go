...
func TestClientAuth(t *testing.T) {
	...
	ts := test_server.StartTestHTTPS(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	ac, err := tls.NewHttpOrHttpsClient(
...
func TestClientAuthExpiredCert(t *testing.T) {
	ts := test_server.StartTestHTTPS(
		...
		test_server.LocalhostCertExpired,
		test_server.LocalhostKeyExpired)
	defer ts.Close()

	ac, err := tls.NewHttpOrHttpsClient(
		tls.Config{ServerCert: "../../common/tls/testdata/server.expired.crt"},
	)
	assert.NotNil(t, ac)
	assert.NoError(t, err)
...
func TestClientAuthUnknownAuthorityCert(t *testing.T) {
	ts := test_server.StartTestHTTPS(
		http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}),
		test_server.LocalhostCertUnknown,
		test_server.LocalhostKeyUnknown)
	defer ts.Close()

	ac, err := tls.NewHttpOrHttpsClient(
		tls.Config{ServerCert: "../../common/tls/testdata/server.unknown-authority.crt"},
	)
	assert.NotNil(t, ac)
	assert.NoError(t, err)
...
func TestClientAuthDepthZeroSelfSignedCert(t *testing.T) {
	...
	ac, err := tls.NewHttpOrHttpsClient(
		tls.Config{ServerCert: "../../common/tls/testdata/server.zero.depth.self.signed.crt"},
	)
	assert.NotNil(t, ac)
	assert.NoError(t, err)
...
func TestClientAuthEndEntityKeyTooSmall(t *testing.T) {
	...
	ts := test_server.StartTestHTTPS(
		http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}),
		test_server.LocalhostCertShortEEKey,
		test_server.LocalhostKeyShortEEKey)
	defer ts.Close()
...
func TestClientAuthNoCert(t *testing.T) {
	...
	ac, err := tls.NewHttpOrHttpsClient(
		tls.Config{ServerCert: "../../common/tls/testdata/server.non-existing.crt"},
	)
	assert.NotNil(t, ac)
	assert.NoError(t, err)
}
...
func TestClientAuthHostValidationError(t *testing.T) {
	ts := test_server.StartTestHTTPS(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}),
		test_server.LocalhostCertWrongHost,
		test_server.LocalhostKeyWrongHost)
	defer ts.Close()
...
func TestClientAuthNotValidCertificate(t *testing.T) {
	...
	ts := test_server.StartTestHTTPS(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}),
		test_server.LocalhostCertCAKeyTooShort,
		test_server.LocalhostKeyCAKeyTooShort)
	defer ts.Close()

	ac, err := tls.NewHttpOrHttpsClient(
		tls.Config{ServerCert: "../../common/tls/testdata/server.ca.key.too.small.crt"},
	)
	assert.NotNil(t, ac)
	assert.NoError(t, err)
